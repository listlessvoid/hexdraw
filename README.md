...
| Key | Action |
|-----|--------|
| `S` | Solve current pattern (playground) |
| `I` | Save PNG + copy to clipboard |
| `J` | Export animated GIF |
| `C` | Copy signature as text |
| `H` | Toggle arc mode |
| `G` | Toggle grid dots |
| `P` / `Shift+P` | Cycle palette / reverse palette |
| `T` / `Shift+T` | Rotate pattern |
| `Tab` | Fit / re-centre view (playground) |
| `Ctrl+Backspace` | Clear pattern (playground) |
| `Ctrl+V` | Paste signature (playground) |
| `1`-`5` | Open/close panels |
| `Left` / `Right` | Previous / next result (gallery) |
| `Backspace` / `ESC` | Return to playground from gallery |
