...
void handlePlaygroundPanel4Mouse() {
  ...
  if (mouseInRect(pgBtnCopyImg))   { copyPlaygroundImage();   return; }
  if (mouseInRect(pgBtnExportGIF)) { exportPlaygroundGIF(); return; }
...
void copyPlaygroundImage() {
  byte[] bytes = sigToBytes(customSignature);
  if (bytes.length == 0) { notify("Nothing to export - pattern is empty", 90, NOTIF_WARN); return; }
  exportPatternCanvas(bytes, -1.0f);
  notify("Image saved + copied to clipboard", 120, NOTIF_SUCCESS);
}

void exportPlaygroundGIF() {
  byte[] bytes = sigToBytes(customSignature);
  if (bytes.length == 0) { notify("Nothing to export - pattern is empty", 90, NOTIF_WARN); return; }
  exportGIF(bytes, frameCount);
}
