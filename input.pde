...
void handlePanel3Mouse() {
  ...
  if (mouseInRect(btnRotMinus)) {
    ...
    if (currentState == AppState.PLAYGROUND || currentState == AppState.CALCULATING) syncSigToPath();
    else resetAnimation(); saveConfig(); return;
  }
...
void handlePanel4Mouse() {
  if (mouseInRect(btnCopySig))    { copyCurrentSignature(); return; }
  if (mouseInRect(btnCopyImg))    { copyCurrentImage();     return; }
  if (mouseInRect(btnExportGIF))  { toggleGIFRecording();   return; }
...
void copyCurrentImage() {
  ...
  synchronized(totalGallery) {
    if (totalGallery.isEmpty()) return;
    if (galleryIndex < 0 || galleryIndex >= totalGallery.size()) return;
    path = totalGallery.get(galleryIndex);
  }
...
void toggleGIFRecording() {
  byte[] activePath;
  synchronized(totalGallery) {
    if (totalGallery.isEmpty()) { notify("Nothing to export", 90, NOTIF_WARN); return; }
    activePath = totalGallery.get(galleryIndex);
  }
  exportGIF(activePath, galleryIndex);
}
...
void keyPressed() {
  ...
  if (currentState == AppState.GALLERY || currentState == AppState.CALCULATING) {
    handleGalleryKey();
  }
}
...
void handlePlaygroundKey() {
  ...
  if (key == '1') { openPanel = (openPanel == 1) ? 0 : 1; return; }
  if (key == '2') { openPanel = (openPanel == 2) ? 0 : 2; return; }
  if (key == '3') { openPanel = (openPanel == 3) ? 0 : 3; return; }
  if (key == '4') { openPanel = (openPanel == 4) ? 0 : 4; return; }
  if (key == '5') { openPanel = (openPanel == 5) ? 0 : 5; return; }

  if (key == '\t') {
    key = 0;
    centerDrawViewOnPath(); // Tab = fit/recenter the pattern in view
    return;
  }
  ...
  if (lc == 'i' && !isModifierForShortcut()) { copyPlaygroundImage(); return; }
  if (lc == 'j' && !isModifierForShortcut()) { exportPlaygroundGIF(); return; }
  if (lc == 'h') {
    arcMode = !arcMode; resetAnimation(); saveConfig();
    notify("Arc mode " + (arcMode ? "ON" : "OFF"), 90, NOTIF_INFO); return;
  }
  if (lc == 'g') {
    gridDotsMode = !gridDotsMode; saveConfig();
    notify("Grid dots " + (gridDotsMode ? "ON" : "OFF"), 90, NOTIF_INFO); return;
  }
  if (lc == 't' && !isModifierForShortcut()) {
    int delta = shiftHeld ? -1 : 1;
    currentPatternStartDir = ((currentPatternStartDir + delta) % 6 + 6) % 6;
    syncSigToPath(); return; // update drawPath orientation to match rotation
  }
  if (lc == 'p') {
    currentPalette = (currentPalette + 1) % (BUILTIN_PALETTE_COUNT + customPalettes.size());
    saveConfig(); return;
  }
  if (lc == 'c' && !isModifierForShortcut()) {
    if (customSignature.length() == 0) { notify("Nothing to copy", 90, NOTIF_WARN); return; }
    try {
      StringSelection sel = new StringSelection(customSignature);
  ...
  if (isModifierForShortcut() && (key == 'z' || key == 'Z')) {
    undoDrawStep(); return;
  }
  // Ctrl+BKS: clear everything (intercept before handleTypeKey)
  if (isModifierForShortcut() && key == BACKSPACE) {
    clearPlayground(); return;
  }
...
void handleGalleryKey() {
  ...
  // ESC closes open panel, then returns to menu
  if (key == ESC) {
  ...
  char lc = Character.toLowerCase(key);
  if (lc == 'o') {
    gotoInputActive = true; gotoInputBuffer = "";
  } else if (lc == 'r') {
    ...
    if (shiftHeld) copyCurrentImage();
    else           copyCurrentSignature();
  } else if (lc == 'i') {
    copyCurrentImage();
  } else if (lc == 'j') {
    toggleGIFRecording();
    ...
    arcMode = !arcMode; resetAnimation(); saveConfig();
    notify("Arc mode " + (arcMode ? "ON" : "OFF"), 90, NOTIF_INFO);
  } else if (lc == 'g') {
    gridDotsMode = !gridDotsMode; saveConfig();
    notify("Grid dots " + (gridDotsMode ? "ON" : "OFF"), 90, NOTIF_INFO);
  }
}
...
void copyCurrentSignature() {
  ...
  try {
    ...
    notify("Copy failed", 90, NOTIF_ERROR);
  }
}
...
void undoDrawStep() {
  if (drawPath.size() <= 0) return;
  int lastDir = drawPath.get(drawPath.size() - 1);
  int prevQ = drawEndAxial[0] - DIR_OFFSETS[lastDir][0];
  int prevR = drawEndAxial[1] - DIR_OFFSETS[lastDir][1];
  String ek = edgeKeyFor(packQR(drawEndAxial[0], drawEndAxial[1]), packQR(prevQ, prevR));
  drawUsedEdges.remove(ek);
  drawPath.remove(drawPath.size() - 1);
  drawEndAxial[0] = prevQ;
  drawEndAxial[1] = prevR;
